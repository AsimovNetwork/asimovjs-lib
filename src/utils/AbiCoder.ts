...
import { NegativeOne, Zero, One, MaxUint256 } from '../Constant';
...
import { deepCopy, defineReadOnly, shallowCopy } from './Property';
...
class CoderAddress extends Coder {
    ...
    encode(value: string): Uint8Array {
        ...
        try {
            result.set(arrayify(getAddress(value)), 11);
        } catch (error) {
    ...
    decode(data: Uint8Array, offset: number): DecodedResult {
        ...
        return {
            consumed: 32,
            value: this.coerceFunc('address', getAddress(hexlify(data.slice(offset + 11, offset + 32))))
       }
    }
}
